...
[System.Serializable]
public class BeerPongBroadcastData
{
...
public class BeerPongServer : NetworkDiscovery {
    public BP_NetworkManager networkManager;
    public BeerPongBroadcastData broadcastDataObject;
    ...
    private void RestartBroadcastInternal()
    {
        ...
        if(networkManager.verboseLogging){
            Debug.Log("#BeerPong# Restarting server with data: " + broadcastData);
        }

        //You cant update broadcastData while the server is running so I have to reinitialize and restart it
        ...
        if(!StartAsServer())
        {
            Debug.LogError("#BeerPong# Unable to broadcast!");
